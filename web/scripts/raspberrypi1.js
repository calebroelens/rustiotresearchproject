...
let init = async () => {
    ...
    let chart = new ApexCharts(document.getElementById("co-chart"), chart_options);
    document.getElementById("co-chart").hidden = true;
    chart.render();
    ...
    window.setInterval(
        async () => {
            if (global_data.length > 0){
                document.getElementById("co-chart").hidden = false;
...
let update_data_by_date = async () => {
    ...
    let fetch_data = await fetch(`device_data/airquality/${year}/${month}/${day}`).then(data => data.json());
...
let handle_new_ping =  (data) =>{
    // Handle the response
    let device_state = data["Airquality"] === 200;
    // Update the HTML
